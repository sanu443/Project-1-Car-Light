...
{   
    static light_mode last_mode_[2] = {OFF};
...
void 
handle_btn_left(OUT_PARAM light_mode* mode, OUT_PARAM btn_state* btn_pressed, IN_OUT_PARAM context* context_)
{
    handle_btn_LR(LEFT_BTN, mode, btn_pressed, context_);
}

void 
handle_btn_right(OUT_PARAM light_mode* mode, OUT_PARAM btn_state* btn_pressed, IN_OUT_PARAM context* context_)
{
...
light_mode
prioritize(light_mode left, light_mode right, light_mode hazard)
{
    ...
    {      
        ...
        mode = (last_mode == LANE_CHANGE_LEFT) ? LANE_CHANGE_RIGHT : LANE_CHANGE_LEFT; //schimbam modul 
    }
    last_mode = mode;
    return mode;
} 
...
normal(u8 LR, context* context_)
{
    ...
    if(millis() - btn.last_release >= 100 &&
        btn.pressed == NOT_PRESSED &&
        context_->last_mode != HAZARD) 
    {
        *off_cmd = TRUE;
    }
}
...
{   
    ...
    light_mode  mode = prioritize(left_mode, right_mode, hazard_mode);
    //DEBUGs
    //MODE
    z(A2, mode % 2);
    z(A3, (mode / 2) % 2);
    ...
    // if (mode == LANE_CHANGE_RIGHT && context_.last_mode == LANE_CHANGE_LEFT)
    // {
    //     z(11);
    // }
