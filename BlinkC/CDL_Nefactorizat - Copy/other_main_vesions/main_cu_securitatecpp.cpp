...
{      
    ...
    {   
        case NOT_PRESSED:
        {
            if (btn_status(HAZARD_BTN_PIN, TEST_PRESS))
            {   
        ...
        case PRESSED:
        {
            if (btn_status(HAZARD_BTN_PIN, TEST_RELEASE))
            {      
...
normal(u8 LR, context* context_)
{
    ...
    digitalWrite_(THE_OTHER_LED, LOW); //TODO: snu e responsabilitatea luia
...
kbscan(void)
{
   ...
   for(DDRD = (1 << 4); DDRD != 0; DDRD <<= 1, lina++) 
   {
      ...
      if (temp) 
      {
         ...
         ;
         return tabela_trans[4 * lina + cola];
      } 
   }
   return NOKEY;
}
