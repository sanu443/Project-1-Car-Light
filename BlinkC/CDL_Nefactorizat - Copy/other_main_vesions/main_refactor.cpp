...
//        = sCOMUNICARE A MODURILOR CU handle_btn, putand pune un mod pe OFF prin off_cmd
//mecanismul off_cmd e useless, trebuia doar sa fac left_mode, right_mode, hazard_mode available din context
typedef struct 
{
    bool_ left_off_cmd, right_off_cmd, hazard_off_cmd;
...
void 
handle_btn_LR(IN_PARAM u8 L_R, 
    OUT_PARAM light_mode* mode, OUT_PARAM btn_state* btn_pressed, IN_OUT_PARAM context* context_)
{   
    static light_mode last_mode_[2] = {OFF}; // nu retin in context last_mode pt fiecare btn
    // OVERHEAD DAR NU DUPLIC CODUL
    light_mode* last_mode = &last_mode_[L_R];
    ...
    bool_* off_cmd = &((L_R == LEFT_BTN) ? context_->left_off_cmd : context_->right_off_cmd);
    ...
    *mode = *last_mode;
    if(*off_cmd == TRUE)
    {   
        *off_cmd = FALSE; // AM RECEPTIONAT COMANDA
        *mode = OFF;
...
void 
handle_btn_left(OUT_PARAM light_mode* mode, OUT_PARAM btn_state* btn_pressed, 
    IN_OUT_PARAM context* context_)
{
    handle_btn_LR(LEFT_BTN, mode, btn_pressed, context_);
}

void 
handle_btn_right(OUT_PARAM light_mode* mode, OUT_PARAM btn_state* btn_pressed,
     IN_OUT_PARAM context* context_)
{
    handle_btn_LR(RIGHT_BTN, mode, btn_pressed, context_);
}

/*TODO: sa updateze din context nu statice last_btn...*/

void 
handle_btn_hazard(OUT_PARAM light_mode* mode, 
    OUT_PARAM btn_state* btn_pressed, IN_PARAM context* context_)
{      

    static light_mode last_mode = OFF;
    static bool_ already_in_hazard = FALSE;

    btn_info* btn = &(context_->hazard_btn);
    btn_state* hazard_pressed = &(btn->pressed);
    u32* last_release = &(btn->last_release);
    u32* last_pressed = &(btn->last_press);
    bool_* off_cmd = &(context_->hazard_off_cmd);
    
    *mode = last_mode;
    if(*off_cmd == TRUE) // AVEAM BUG if(off_cmd), am stat 3h
    {
        *mode = OFF;
        *off_cmd = FALSE; //INAINTE UITAM SA DAU CLEAR LA COMANDA
        digitalWrite(LEFT_LED, LOW);
        digitalWrite(RIGHT_LED, LOW);
    }
        ...
        case PRESSED:
        {
            ...
            {      
                ...
                *last_release = millis();
                ...
                {      
                    *mode = HAZARD;
                    *off_cmd = already_in_hazard;
                    already_in_hazard = !already_in_hazard;
                }
            }
        }
        break;
    }
    *btn_pressed = *hazard_pressed;
    last_mode = *mode;
}

void
turn_off_mode(light_mode mode, IN_PARAM context* context_)
{
    switch(mode)
    {
        ...
        case NORMAL_LEFT:
        {
            context_->left_off_cmd = TRUE;
        }
        break; // nu aveam break, alt bug la care am stat 2 ore
        case LANE_CHANGE_RIGHT:
        case NORMAL_RIGHT:
        {
            context_->right_off_cmd = TRUE;
        }
        break;
        case HAZARD:
        {
            context_->hazard_off_cmd = TRUE;
        }
        break;
        default:
        break;
...
light_mode
prioritize(light_mode left, light_mode right, light_mode hazard, context* context_)
{
    //static light_mode last_left = OFF, last_right = OFF, last_hazard = OFF;
    //static light_mode last_mode = OFF;
    light_mode mode;
    light_mode highest_priority_mode =  max3(left, right, hazard);
    
    mode = highest_priority_mode;
    if (left == LANE_CHANGE_LEFT && right == LANE_CHANGE_RIGHT && hazard != HAZARD)
    {      
...
lane_change(u8 LR, context* context_)
{
    ...
    bool_* off_cmd = &((LR == LEFT_BTN) ? context_->left_off_cmd : context_->right_off_cmd);
...
{   
    ...
    {
        digitalWrite(LEFT_LED, LOW); //TODO: din nou celelate modui trb sa dea cleanup
        digitalWrite(RIGHT_LED, LOW); //nu noi la inceput
    }
...
{   
    ...
    u32 last_exec_times[NR_MAX_TASKS];
    u8 crt_task;
} scheduler;
#undef NR_MAX_TASKS
...
{   
    ...
    context_->left_off_cmd = FALSE;
    context_->right_off_cmd = FALSE;
    context_->hazard_off_cmd = FALSE;
...
{   
    ...
    static light_mode left_mode = OFF, right_mode = OFF, hazard_mode = OFF;
    handle_btn_left(&left_mode, &(context_.left_btn.pressed), &context_);
    handle_btn_right(&right_mode, &(context_.right_btn.pressed), &context_);
    handle_btn_hazard(&hazard_mode, &(context_.hazard_btn.pressed), &context_);
    ...
    light_mode  mode = prioritize(left_mode, right_mode, hazard_mode, &context_);
    //DEBUG
    //MODE
    z(A2, mode & 1);
    z(A3, (mode >> 1) & 1);
    ...
    print(&lcd, mode, context_.left_btn.pressed, context_.right_btn.pressed, context_.hazard_btn.pressed);
